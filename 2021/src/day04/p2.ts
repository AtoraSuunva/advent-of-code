...
let nextNumber = 0
let loser: BingoBoard | undefined

while (boards.some((board) => !board.isBingo())) {
  const tmp = numbers.shift()

  if (tmp === undefined) {
    throw new Error('No more numbers, and no single loser')
  }

  nextNumber = tmp
  boards.forEach((board) => board.markNumber(nextNumber))

  if (boards.filter((board) => !board.isBingo()).length === 1) {
    // One board left, it's the loser
    loser = boards.find((board) => !board.isBingo())
    // We still need to continue until it wins though
  }
}
...
const unmarked = loser.getUnmarkedNumbers()
